...
test('随机外观组件只替换模型并保留顾客购买链路节点', () => {
    ...
    assert.match(source, /child\.getComponentInChildren\(SkeletalAnimation\)/);
    assert.match(source, /child\.getComponentInChildren\(MeshRenderer\)/);
...
test('森林和两个玉米顾客队伍均绑定同一组五种随机外观资源', () => {
    ...
    for (const { entry, index } of randomizers) {
        ...
        assert.equal(entry.customerModelScale, 1.1);
