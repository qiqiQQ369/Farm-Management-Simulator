...
const component = vue_1.default.extend({
    template: (0, fs_extra_1.readFileSync)((0, path_1.join)(__dirname, '../static/app.html'), 'utf-8'),
    data() {
        return {
            menuItems: [
            ...
            menuIndex: 0,
            nodesMeshMergeDatas: [{ uuid: "", childs: true }],
            subMeshSplitUUid: "",
    ...
    methods: {
        ...
        onOperateMeshForce(force) {
            this.meshIsForce = force;
        },
        onOperateMeshWorld(world) {
            this.meshIsWorld = world;
        },
        onOperateMegerTerrain(mergeTerrain) {
        ...
        bindNodeMeshMergeNode(index, uuid) {
            let i = Number(index);
            this.nodesMeshMergeDatas[i].uuid = uuid;
        },
        changeNodeMeshMergeNodeChilds(index, childs) {
            let i = Number(index);
            this.nodesMeshMergeDatas[i].childs = childs;
        },
        delNodeMeshMerge(index) {
            let i = Number(index);
            this.nodesMeshMergeDatas.splice(i, 1);
        },
        /***************************选中节点合并* end*********************************** */
        /***************************子网格拆分* start*********************************** */
        async bindSubMeshSplit(uuid) {
            this.subMeshSplitDatas = [];
            ...
            let subMeshLen = await Editor.Message.request('scene', 'execute-scene-script', options);
        ...
        onSubMeshSplitSelect(index, select) {
            let i = Number(index);
        ...
        async bindUVLookNode(uuid) {
            ...
            let data = await Editor.Message.request('scene', 'execute-scene-script', options);
            // console.log(data)
            if (data.length > 4) {
        ...
        setMeshAndUVList() {
            let subLen = this.uvDatas["len"];
            if (subLen > 0) {
                for (let i = 0; i < subLen; i++) {
                    this.uvSubMesh.push({ label: "子网格" + (i + 1), value: i });
                }
                let uvs = this.uvDatas["sub"][0];
                let uvLen = uvs["len"];
                if (uvLen > 0) {
        ...
        canvasMouseTouchEnd(event) {
            this.uvPanelTouch = false;
        },
        canvasMouseWheel(event) {
            //根据放大和缩小的比例，重新计算坐标中心点的位置
            let _centerX = this.uvCenterX - this.uvcanvasW / 2.0;
            let _centerY = this.uvCenterY - this.uvcanvasH / 2.0;
            let scale = 0.2;
        ...
        renderUv() {
            if (!this.uvDatas)
                return;
            if (this.uvSubMeshIndex < 0)
                return;
            if (this.uvSubUVIndex < 0)
                return;
            let subMeshIndex = this.uvSubMeshIndex;
            let subUVIndex = this.uvSubUVIndex;
            let subUVs = this.uvDatas["sub"];
            if (subMeshIndex < subUVs.length) {
                let uvs = subUVs[subMeshIndex]["uv"];
                if (subUVIndex < uvs.length) {
                    let uv = uvs[subUVIndex];
        ...
        renderLine(points, lineWidth, color) {
        ...
        renderText(text, x, y, fontSize, fontColor) {
...
module.exports = Editor.Panel.define({
    ...
    template: (0, fs_extra_1.readFileSync)((0, path_1.join)(__dirname, '../static/index.html'), 'utf-8'),
    style: (0, fs_extra_1.readFileSync)((0, path_1.join)(__dirname, '../static/index.css'), 'utf-8'),
    $: {
        app: '#app',
    },
    methods: {
