 ...
 */
export class MeshUV {
    private static _ATTR_TEX_COORD = ["a_texCoord", "a_texCoord1", "a_texCoord2", "a_texCoord3", "a_texCoord4", "a_texCoord5", "a_texCoord6", "a_texCoord7", "a_texCoord8"];
    /**
     * 获取模型的uv数据
     */
    static getMeshUv(uuid: string): any {
        let node = cce.Node.query(uuid);
        if (node) {
            let compMeshRender: any = node.getComponent(MeshRenderer);
            if (compMeshRender) {
    ...
    private static getUvsData(meshRenderer: any): any {
        ...
        for (let i = 0; i < primitives.length; i++) {
            ...
            let is: any = indices ? Array.from(indices) : null;
            if (!is) {
            ...
            for (let u = 0; u < _uvs.length; u++) {
                ...
                for (let j = 0; j < is.length; j++) {
                    let p = is[j];
                    let uvX: number = _uv[p * 2];
                    let uvY: number = _uv[p * 2 + 1];
                    //console.log("::"+uvY);
